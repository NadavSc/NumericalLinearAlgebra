...
def theoretical_error(lr_s_values, k):
    return np.sqrt(np.sum(lr_s_values[k:]**2))
...
if __name__ == '__main__':
    ...
    if section == 1:
        ...
        for tau in tau_values:
            ...
            calc_time = np.mean(timeit.repeat(lambda: low_rank_approximation(A, tau), repeat=3, number=1))
            ranks.append(rank)
            errors.append(compute_error(A, A_lr))
            theoretical_errors.append(theoretical_error(lr_s_values, rank))
